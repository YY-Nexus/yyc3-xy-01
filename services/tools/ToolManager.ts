/**
 * @file ToolManager.ts
 * @description YYC³ AI小语智能成长守护系统工具管理器，统一管理工具注册、发现、执行和编排
...
export class ToolManager extends EventEmitter {
  ...
  getToolMetrics(toolName: string) {
    ...
    return this.toolRegistry.getToolMetrics(toolName)
  }

  /**
   * 获取编排状态
   */
  getOrchestrationStatus(planId: string) {
    if (!this.isInitialized) {
      throw new Error('工具管理器未初始化')
    }

    return this.toolOrchestrator.getExecutionStatus(planId)
  }

  /**
   * 取消编排
   */
  async cancelOrchestration(planId: string, reason?: string): Promise<boolean> {
    if (!this.isInitialized) {
      throw new Error('工具管理器未初始化')
    }

    return await this.toolOrchestrator.cancelExecution(planId, reason)
  }

  /**
   * 重试编排
   */
  async retryOrchestration(planId: string): Promise<string> {
    if (!this.isInitialized) {
      throw new Error('工具管理器未初始化')
    }

    return await this.toolOrchestrator.retryExecution(planId)
  }
  ...
  private calculateRelevanceScore(
    tool: ToolDefinition,
    goal: string,
    _context?: ToolRecommendationContext
  ): number {
    let score = 0
